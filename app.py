...
settings_data = {'fingerprint_timeout': 10}  # Default setting
fingerprint_controller = FingerprintController(port="/dev/ttyUSB0")
...
class KeypadController:
    ...
    def start(self):
        ...
        if not self.running:
            self.thread = threading.Thread(target=self.run_keypad_loop, daemon=True)
            self.thread.start()
    ...
    def check_passcode(self):
        ...
        if not self.current_input:
            print("âŒ No passcode entered on keypad")
            return
        ...
        if self.current_input == system_passcode:
            ...
            print("âŒ Incorrect keypad passcode")
    ...
    def run_keypad_loop(self):
        """Continuously scan the keypad in a separate thread"""
        print("🟢 Keypad input loop started")
        self.running = True
        while self.running:
            try:
                ...
                print(f"🛑 Keypad loop error: {e}")
            time.sleep(0.1)  # debounce / avoid tight loop
        ...
        try:
            ...
            def ai_conversation_thread():
                try:
                    ...
                    print(f"âŒ AI conversation error: {e}")
...
def fingerprint_authenticate():
    if not fingerprint_controller or not fingerprint_controller.available:
        return jsonify({'success': False, 'message': 'Fingerprint sensor not available'})
        ...
        if result['success']:
            ...
            if username and username in users_data:
                ...
                if not user_data.get('approved', False):
                    ...
                    return jsonify({'success': False, 'message': 'User account not approved'})
                ...
                add_log('fingerprint', 'success', username, 'Authentication successful')

                # 🔓 Unlock door and log
                door_controller.unlock()
                add_log('door', 'unlocked', username, 'Door unlocked after successful fingerprint')
                ...
                return jsonify({'success': False, 'message': 'Fingerprint matched but user not found'})
        else:
...
def continuous_fingerprint_auth():
    print("🟢 Fingerprint authentication loop started")
    while True:
        try:
            with fingerprint_lock:
                result = fingerprint_controller.authenticate_fingerprint(users_data, timeout=1)

            if result['success']:
                username = result.get('username')
                print(f"✅ Fingerprint matched: {username}")
                
                # 🔓 Unlock door and log
                door_controller.unlock()
                add_log('fingerprint', 'success', username, 'Auto-auth success via continuous scan')
                add_log('door', 'unlocked', username, 'Door auto-unlocked via fingerprint')

                # Optional: auto-lock after 10 seconds
                time.sleep(10)
                door_controller.lock()
                add_log('door', 'locked', username, 'Door auto-locked after unlock delay')
        except Exception as e:
            print(f"🛑 Fingerprint thread error: {e}")
        time.sleep(3)  # To prevent tight loop
    ...
    try:
        ...
        if result['success']:
            ...
            add_log('fingerprint', 'enrolled', current_username,
                    f'Enrollment successful for {target_username}, slot {result.get("slot_id")}')

            return jsonify({'success': True, 'message': 'Fingerprint enrolled', 'slot_id': result.get('slot_id')})
        else:
            add_log('fingerprint', 'enrollment_failed', current_username, result.get("message"))
            return jsonify(result)

    except Exception as e:
        print(f"Error: {e}")
        add_log('fingerprint', 'error', 'system', str(e))
        return jsonify({'success': False, 'message': f'Enrollment error: {str(e)}'})

@app.route('/api/fingerprint/sensor/info')
@login_required
def fingerprint_sensor_info():
    if not fingerprint_controller:
        return jsonify({'success': False, 'message': 'Controller not available'})
...
def test_fingerprint_sensor():
    ...
    try:
        ...
        add_log('fingerprint', 'test', session.get('username'), result.get('message'))
        return jsonify(result)
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})
...
def fingerprint_delete():
    try:
        ...
        target_user.pop('fingerprint_enrolled_date', None)
        target_user.pop('fingerprint_enrolled_by', None)
        target_user.pop('fingerprint_slot_id', None)

        save_users()
        add_log('fingerprint', 'deleted', current_username, f'Deleted for {target_username}')
        return jsonify({'success': True, 'message': 'Fingerprint data deleted'})

    except Exception as e:
        print(f"Error: {e}")
        add_log('fingerprint', 'error', 'system', str(e))
        return jsonify({'success': False, 'message': 'Deletion error'})
...
def fingerprint_status(username):
    try:
        ...
        print(f"Error: {e}")
        return jsonify({'success': False, 'message': 'Status error'})
...
if __name__ == '__main__':
    try:
        ...
        # Start background fingerprint authentication thread
        Thread(target=continuous_fingerprint_auth, daemon=True).start()
