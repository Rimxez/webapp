        ...
        def ai_conversation_thread():
            try:
                ...
                if self.conversation_log:
                    ...
                    self.log_to_system('ai_conversation', 'complete_transcript', 'keypad', conversation_summary)
                
                self.log_to_system('ai', 'interaction_ended', 'keypad', 'AI conversation session ended')
        ...
        try:
            ...
            for entry in self.conversation_log:
                ...
                if speaker == 'VISITOR' and confidence > 0:
                    ...
                    if len(text.split()) <= 3 and any(word in text.lower() for word in ['i am', 'my name', 'this is']):
                        visitor_name = text.strip()
                    elif len(text) > 10:  # Longer responses likely contain purpose
                        visit_purpose = text.strip()
                        detected_intent = self.analyze_intent(text)
            ...
            summary_parts.append(f"Conversation Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            summary_parts.append(f"Average Speech Confidence: {avg_confidence:.2f}")
            summary_parts.append(f"Total Conversation Exchanges: {len(self.conversation_log)}")
            summary_parts.append("")
            summary_parts.append("--- COMPLETE CONVERSATION TRANSCRIPT ---")
            
            # Add full conversation transcript with numbering
            ...
            summary_parts.append("")
            summary_parts.append("--- CONVERSATION QUALITY ANALYSIS ---")
            summary_parts.append(f"• Visitor Name Captured: {'✅ Yes' if visitor_name != 'Unknown Visitor' else '❌ No'}")
            summary_parts.append(f"• Visit Purpose Identified: {'✅ Yes' if visit_purpose != 'Not specified' else '❌ No'}")
            summary_parts.append(f"• Intent Classification: {detected_intent}")
            ...
            summary_parts.append(f"• Speech Recognition Quality: {quality} ({avg_confidence:.2f})")
            summary_parts.append(f"• Conversation Flow: ✅ Completed Successfully")
            summary_parts.append(f"• System Response: ✅ Residents Notified")
            summary_parts.append("")
            summary_parts.append("--- RECOMMENDED ACTIONS ---")
            
            if detected_intent == 'delivery':
                summary_parts.append("• 📦 Package delivery - Check for packages")
            elif detected_intent == 'visit':
                summary_parts.append("• 👥 Social visit - Visitor waiting for resident")
            elif detected_intent == 'service':
                summary_parts.append("• 🔧 Service call - Technician/maintenance visit")
            elif detected_intent == 'emergency':
                summary_parts.append("• 🚨 URGENT - Emergency situation requires immediate attention")
            else:
                summary_parts.append("• ℹ️ General visit - Standard visitor protocol applied")
            
            summary_parts.append("=== END COMPLETE TRANSCRIPT ===")
            
            return "\n".join(summary_parts)
            
        except Exception as e:
            return f"Error compiling comprehensive conversation summary: {str(e)}"
