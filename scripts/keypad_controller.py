...
class KeypadController:
    ...
    def start_ai_interaction(self):
        ...
        try:
            ...
            if hasattr(self, 'conversation_log') and self.conversation_log:
                conversation_summary = self._compile_conversation_summary()
                add_log('ai_conversation', 'complete_transcript', 'keypad', conversation_summary)
    ...
    def _compile_conversation_summary(self):
        """Compile all conversation data into a comprehensive summary"""
        try:
            summary_parts = []
            ...
            for entry in self.conversation_log:
                ...
                if speaker == 'VISITOR' and confidence > 0:
                    confidence_count += 1
                    total_confidence += confidence
                    
                    # Try to identify if this is the name or purpose
                    if 'name' in text.lower() or len(text.split()) <= 3:
                        visitor_name = text.strip()
                    else:
                        visit_purpose = text.strip()
                        detected_intent = self.analyze_visitor_intent(text)
        
            # Calculate average confidence
            avg_confidence = (total_confidence / confidence_count) if confidence_count > 0 else 0
            
            # Build comprehensive summary
            summary_parts.append(f"=== COMPLETE AI CONVERSATION TRANSCRIPT ===")
            summary_parts.append(f"Visitor Name: {visitor_name}")
            summary_parts.append(f"Visit Purpose: {visit_purpose}")
            summary_parts.append(f"Detected Intent: {detected_intent}")
            ...
            summary_parts.append("--- FULL CONVERSATION TRANSCRIPT ---")
            ...
            for i, entry in enumerate(self.conversation_log, 1):
                ...
                speaker_icon = "🤖" if speaker == 'AI' else "👤"
                confidence_str = f" (confidence: {confidence:.2f})" if confidence > 0 and speaker == 'VISITOR' else ""
                
                summary_parts.append(f"{i:2d}. [{timestamp}] {speaker_icon} {speaker}: {text}{confidence_str}")
            
            summary_parts.append("")
            summary_parts.append("--- CONVERSATION ANALYSIS ---")
            summary_parts.append(f"• Visitor successfully identified: {'Yes' if visitor_name != 'Unknown Visitor' else 'No'}")
            summary_parts.append(f"• Purpose clearly understood: {'Yes' if visit_purpose != 'Not specified' else 'No'}")
            summary_parts.append(f"• Speech recognition quality: {'Excellent' if avg_confidence > 0.8 else 'Good' if avg_confidence > 0.6 else 'Fair' if avg_confidence > 0.4 else 'Poor'}")
            summary_parts.append(f"• Conversation completed successfully: Yes")
            summary_parts.append("=== END TRANSCRIPT ===")
            
            return "\n".join(summary_parts)
        
        except Exception as e:
            return f"Error compiling conversation summary: {str(e)}"
